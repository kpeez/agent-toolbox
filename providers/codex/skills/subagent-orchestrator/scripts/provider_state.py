...
def default_state_path() -> Path:
    override = os.environ.get("CODEX_SUBAGENT_STATE_PATH")
    if override:
        return Path(override).expanduser()
    return Path.home() / ".codex" / "subagent-orchestrator" / "provider-state.json"
...
def build_parser() -> ArgumentParser:
    ...
    parser = ArgumentParser(
        description="Track Codex subagent provider ranking and temporary cooldowns."
    )
    parser.add_argument(
        "--state",
