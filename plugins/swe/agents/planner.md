...
2. Gather any codebase evidence you need — the project's vocabulary, the ADRs
   covering the area, where the work lands — by calling an available
   host-native explorer subagent with one bounded question and the workspace
   root. Fall back to your own `Read`/`Grep`/`Glob` only if that subagent is
   unavailable.
