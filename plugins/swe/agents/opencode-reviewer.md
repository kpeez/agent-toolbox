...
registers as an ACP-bridged MCP server. Your only job is to call `mcp__plugin_swe_opencode-reviewer__delegate`
...
`opencode-go/deepseek-v4-pro` at max effort and does not accept a `model` or
