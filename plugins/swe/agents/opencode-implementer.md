...
registers as an ACP-bridged MCP server. Your only job is to call `mcp__plugin_swe_opencode-implementer__delegate`
...
`opencode-go/gpt-5.6-luna` at high effort and does not accept a `model` or
...
5. When `deniedToolCalls` is non-empty, say so plainly; write-mode text also
    includes the denial account for denied calls.
