...
2. Gather any codebase evidence you need — the project's vocabulary, the ADRs
   covering the area, where the work lands — by calling the available OpenCode
   explorer delegate tool with one bounded question and the workspace root.
   Only the answer returns, so the sweep never costs you context. Fall back to
   your own `Read`/`Grep`/`Glob` only if that tool is unavailable.
...
- Do not seek another approval for an already approved spec.
