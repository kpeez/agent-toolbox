...
their own list: in one campaign, all three were real delivery bugs, not stale
tests.
...
prefixes: for a messaging integration, accounts, commands, dispatch, inbound,
outbound, persistence, transport, shared support, and live or QA scenarios.
...
progress window asserted that the window was _not_ cleared.
...
pass, starting from the ledger, looks for the redundant **layer**. In one
campaign, several dispatch suites replayed the same shared compositor through
one mocked preview, around stronger real-stream and HTTP-fixture suites. Name
the **keeper** suite for each contract. Prefer the real transport boundary with
a fake network over a mocked collaborator. Correct any ledger errors this pass
finds.
...
Update CI configuration, test inventories, and any size baselines for moved or
retired suites. Put durable test-ownership rules in the subsystem's agent
instructions, drawn only from mistakes this campaign actually found.
...
keeper, and confirm every new regression it added still has a home. Rerun the
...
maintainer decisions about gates in the PR evidence rather than weakening the
gates.
...
- preservation gaps found and their caught mutations;
